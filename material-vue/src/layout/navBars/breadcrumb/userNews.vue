...
export default defineComponent({
	...
	setup(props, {emit}) {
		...
		let isShow: boolean = state.newsList.length > 0;
		emit('isBot', isShow);
		watch(state.newsList, (newvalue, oldvalue)=>{
				isShow = newvalue.length > 0;
				emit('isBot', isShow);
		})
		// 全部已读点击
		...
		const onGoToGiteeClick = () => {
			window.open('https://gitee.com/hidewnd/material-system');
		};
		return {
			onAllReadClick,
			onGoToGiteeClick,
