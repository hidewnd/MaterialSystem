<template>
	<div class="resources-material-material-type">
		<el-card shadow="never" :header="$t('message.router.materialMaterialType')">
			<div class="resources-system-search mb15">
				<el-form :model="tableData.param" size="default" label-width="90px">
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label-width="auto">
							<el-input placeholder="请输入物料类型系统名" style="max-width: 180px" v-model="tableData.param.arg2" clearable></el-input>
						</el-form-item>
					</el-col>
				</el-form>
			...
			<el-table :data="tableData.data" style="width: 100%">
				...
				<el-table-column prop="typeId" label="物料类型编号" show-overflow-tooltip sortable></el-table-column>
				<el-table-column prop="parentId" label="父级编号" show-overflow-tooltip sortable></el-table-column>
				<el-table-column prop="typeName" label="物料类型系统名" show-overflow-tooltip></el-table-column>
				<el-table-column prop="typeNameZh" label="物料类型中文名" show-overflow-tooltip></el-table-column>
		...
		<add-material-type ref="addRef" @refresh="refreshTable" />
		<edit-material-type ref="editRef" @refresh="refreshTable" />
	</div>
</template>
...
import AddMaterialType from './component/ResourcesAddMaterialType.vue';
import EditMaterialType from './component/ResourcesEditMaterialType.vue';
import qs from 'qs';

interface TableDataState {
	tableData: {
		data: Array<MaterialType>;
		total: number;
		loading: boolean;
		param: {
			type: string;
...
export default defineComponent({
	name: 'ResourcesMaterialMaterialType',
	components: { AddMaterialType, EditMaterialType },
	setup() {
		const addRef = ref();
		const editRef = ref();
		...
		// 打开修改页面
		function onOpenEditPage(row: MaterialType) {
		...
		// 行删除
		function onRowDel(row: MaterialType) {
			ElMessageBox.confirm(`此操作将永久删除“${row.typeName}”？是否继续?`, '提示', {
		...
		function setTableDataByList(data: any) {
			const array: Array<MaterialType> = [];
