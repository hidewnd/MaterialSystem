<template>
	<div class="system-Menu">
		<el-card shadow="never" :header="$t('message.router.systemMenu')">
			<div class="resources-system-search mb15">
				<el-form :model="searchParam" size="default" label-width="90px">
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label-width="auto">
							<el-input placeholder="请输入权限名称" style="max-width: 180px" v-model="searchParam.arg" clearable></el-input>
						</el-form-item>
					</el-col>
				</el-form>
			...
			<el-table :data="tableData.data" style="width: 100%">
				...
				<el-table-column prop="menuId" label="权限编号" show-overflow-tooltip sortable></el-table-column>
				<el-table-column prop="parentId" label="父级编号" show-overflow-tooltip sortable></el-table-column>
				<el-table-column prop="name" label="权限名" show-overflow-tooltip></el-table-column>
				<el-table-column prop="url" label="请求路径" show-overflow-tooltip></el-table-column>
				<el-table-column prop="perms" label="权限符" show-overflow-tooltip></el-table-column>
		...
		<add-menu ref="addRef" @refresh="refreshTable" />
		<edit-menu ref="editRef" @refresh="refreshTable" />
	</div>
</template>
	...
	setup() {
		...
		const state = reactive<TableDataState>({
			tableData: {
				...
				disable: ['sys', 'default', 'auth', 'mat', 'manager'],
			},
			searchParam: {
				name: 'menu',
				type: 'like',
		...
		// 打开修改页面
		function onOpenEditPage(row: SysMenu) {
		...
		// 行删除
		function onRowDel(row: SysMenu) {
			ElMessageBox.confirm(`此操作将永久删除“${row.menuId}”权限？是否继续?`, '提示', {
				confirmButtonText: '确认',
				cancelButtonText: '取消',
				type: 'warning',
			})
				.then(() => {
					if (['sys', 'default', 'auth', 'mat', 'manager'].indexOf(row.menuId) >= 0) {
						ElMessage.error('该权限无法删除');
						return;
					}
					SystemMenuApi()
						.setMeu('type=delete&menuId=' + row.menuId + '&perms=' + row.perms)
						.then((res) => {
