	...
	setup() {
		...
		function initLineChart() {
			...
			const option = {
				...
				legend: { data: ['物料入库', '物料出库'], right: 0 },
				xAxis: {
					data: state.recordTypeTotal.typeName,
				},
				yAxis: [
					{
						type: 'value',
		...
		function initPieChart() {
			...
			var getname = state.typeTotal.typeName;
			var getvalue = state.typeTotal.number;
			var data = [];
		...
		function initData() {
			if (state.stockTotal.applyEnterTotal === '') {
				StatisticAPI()
					.queryStockTotal()
					.then((res) => {
						...
						if (data) {
							state.homeOne[0].num1 = data.materialTotal;
							state.homeOne[1].num1 = data.applyEnterTotal;
							state.homeOne[2].num1 = data.applyOutTotal;
							state.stockTotal = {
					...
					StatisticAPI()
						...
						.then((res) => {
							...
							if (data) {
								...
								state.homeOne[3].num1 = data.total;
							}
						});
				}
			}, 100);
			setTimeout(() => {
				if (state.typeTotal.number.length === 0) {
					StatisticAPI()
						.queryTypeStockTotal()
						.then((res) => {
							let data: any = JSON.stringify(res.data);
			...
			setTimeout(() => {
				if (state.recordTypeTotal.enterNumber.length === 0) {
					StatisticAPI()
						...
						.then((res) => {
							...
							if (data) {
								...
								state.recordTypeTotal.typeName = data.typeName;
							}
						});
				}
			}, 700);
		...
		function initUserInfo() {
			...
			ResourceSystemApi()
				.queryOne({ name: 'info', type: 'id', arg1: userInfo.userId })
				.then((res) => {
					let info: any = JSON.stringify(res.data);
					info = JSON.parse(info);
					if (info && info.avatar != undefined && verifyUrl(info.avatar)) {
						userInfo.photo = info.avatar;
